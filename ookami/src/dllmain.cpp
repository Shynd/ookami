...
using FrameScript__ExecuteBuffer_Fn = void(__fastcall*)(const char*, const char*);

void PrintChat(const char* string)
{
  char buffer[256];
  snprintf(buffer, sizeof(buffer), "DEFAULT_CHAT_FRAME:AddMessage('%s', 0.0, 1.0, 0.0)", string);
...
DWORD WINAPI OnDllAttach(LPVOID lpvBase)
{
  ...
  memory_operations["FrameScript__SignalEvent"] = new Detour(0x00703E50, (int)SignalEventDetour);
