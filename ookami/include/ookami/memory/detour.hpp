...
public:
  Detour(int target_func, int hook_func)
    : MemoryOperation((byte*)target_func), hook{hook_func}
  {
  ...
  ~Detour() { Detour::Restore(); }

  void Apply() { memcpy(target, new_bytes.data(), 6); }

  void Restore() { memcpy(target, original_bytes.data(), 6); }
